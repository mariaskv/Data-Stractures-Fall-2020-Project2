...
static SetNode node_insert(SetNode root,SetNode node, CompareFunc compare, Pointer value, bool* inserted, Pointer* old_value) {
	...
	if (compare_res == 0) {
		...
		*old_value = node->value;
		node->value = value;
		// node->next = node_find_next(root,compare,node);
		// node->prev = node_find_previous(root,compare,node);

	} else if (compare_res < 0) {
		// value < node->value, συνεχίζουμε αριστερά.
		node->left = node_insert(root,node->left, compare, value, inserted, old_value);

	} else {
		// value > node->value, συνεχίζουμε δεξιά
		node->right = node_insert(root,node->right, compare, value, inserted, old_value);
	}
...
void set_insert(Set set, Pointer value) {
	...
	bool inserted;
	Pointer old_value;
	set->root = node_insert(set->root, set->root, set->compare, value, &inserted, &old_value);
	...
	set->first = node_find_min(set->root);
	set->last = node_find_max(set->root);

	for(SetNode new = set_first(set);new!=SET_BOF;new=node_find_next(set->root,set->compare,new)){
		new->next = node_find_next(set->root,set->compare,new);
		new->prev = node_find_previous(set->root,set->compare,new);
	}
}
